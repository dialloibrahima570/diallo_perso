...
@section('title', 'Détail demande projet')
...
.card-header { font-weight:600; font-size:16px; color: #ff2e2e; }
...
<div class="header">
    ...
    <h1>Répondre à la demande projet</h1>
</div>
...
<div class="main-container">
    ...
    <!-- Infos Projet -->
    <div class="mb-4 card">
        <div class="card-header">Informations de la demande</div>
        <div class="card-body">
            <p><strong>Nom :</strong> {{ $project->name }}</p>
            <p><strong>Email :</strong> {{ $project->email }}</p>
            <p><strong>Projet demandé :</strong> {{ $project->project }}</p>
            <p><strong>Message :</strong></p>
            <div class="p-3 border rounded bg-light">{{ $project->message }}</div>

            <p class="mt-3">
                <strong>Statut :</strong>
                <span class="badge {{ $project->status === 'accepted' ? 'approved' : ($project->status === 'rejected' ? 'rejected' : 'pending') }}">
    ...
    <!-- Formulaire envoi -->
    <div class="card">
        <div class="card-header">Envoyer le projet</div>
        <div class="card-body">
            <form action="{{ route('admin.requests.project.send', $project->id) }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-3">
                    <label class="form-label">Message (optionnel)</label>
                    <textarea name="message" class="form-control" rows="4" placeholder="Message qui accompagnera le projet..."></textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label">Lien du projet (Drive, GitHub, etc.)</label>
                    <input type="url" name="project_link" class="form-control" placeholder="https://">
                </div>

                <button type="submit" class="btn-red"><i class="bi bi-send"></i> Envoyer le projet</button>
            </form>
        </div>
    </div>
